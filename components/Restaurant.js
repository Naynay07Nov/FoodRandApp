...
export default function Restaurant(props) {
    ...
    const loadOnlineTours = async () => {
        try {
            let promise = await fetch('https://raw.githubusercontent.com/Naynay07Nov/FoodRandApp/main/data.json');
    ...
    return (
        <View style={props.style}>
            <View style={{ marginRight:270,marginLeft:5 ,alignItems: 'flex-start', flex: 1, marginTop: 10, padding: 1, backgroundColor: '#ffc77a', borderRadius: 15}}>
            <Text style={{ fontSize: 20 ,padding:10}}>Thai Food</Text>
            </View>
            <FlatList
                horizontal={true}
                data={onlineTours}
