...
export default function RestaurantC(props) {

    const [onlineTours, setOnlineTours] = useState([]);
    const loadOnlineTours = async () => {
        try {
            let promise = await fetch('https://raw.githubusercontent.com/Naynay07Nov/FoodRandApp/main/DataChineseRestaurant');
    ...
    return (
        <View style={props.style}>
             <View style={{ marginRight:240,marginLeft:5 ,alignItems: 'flex-start', flex: 1, marginTop: 20, padding: 1, backgroundColor: '#ff9999', borderRadius: 15}}>
            <Text style={{ fontSize: 20 ,padding:10}}>Chinese Food</Text>
            </View>
            <FlatList
                horizontal={true}
                data={onlineTours}
